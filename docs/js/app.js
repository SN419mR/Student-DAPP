App = {
  ...
  loadContract: async () => {
    ...
    const Sample = await $.getJSON('Sample2.json')
  ...
  render: async () => {
   var count=await App.sample.total();
   //window.alert(count)
   $("#display").empty()
   for(var i=1;i<=parseInt(count);i++){
      var s=await App.sample.students(parseInt(i))
      var str ="<tr><td>"+s[0]+"</td><td>"+s[1]+"</td><td>"+s[2]+"</td></tr>";
      $("#display").append(str)
   }
  } ,
  addStudent: async ()=>{
    ...
    var p=$("#b2").val()
    var e=$("#b3").val()
    await App.sample.addStudent(n,p,e,{from:App.account});
    await App.render();
  }
}
